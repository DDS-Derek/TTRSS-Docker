...
COPY --chmod=755 src/wait-for.sh /wait-for.sh
COPY --chmod=755 src/ttrss.nginx.conf /etc/nginx/nginx.conf
COPY --chmod=755 src/initialize.php /initialize.php
COPY --chmod=755 src/s6/ /etc/s6/
...
RUN addgroup -S ttrss -g 911 && \
    adduser -S ttrss -G ttrss -h /app -u 911 && \
    usermod -s /bin/bash ttrss && \
    sed -i "s/user = nobody/user = ttrss/g" /etc/php81/php-fpm.d/www.conf && \
    sed -i "s/group = nobody/group = ttrss/g" /etc/php81/php-fpm.d/www.conf

ENV LD_PRELOAD /usr/lib/preloadable_libiconv.so php

# Copy TTRSS and plugins
COPY --chmod=755 --from=builder /var/www /var/www
